...
%   $Revision: 1.1.6.5 $  $Date: 2011/05/13 18:10:55 $ 

if ~isfir(Hd),
    error(message('signal:dfilt:multistage:thisfirtype:expectFIR'));
end

if ~islinphase(Hd),
    error(message('signal:dfilt:multistage:thisfirtype:expectLinearPhase'));
end

if ~isreal(Hd),
    error(message('signal:dfilt:multistage:thisfirtype:MustBeReal'));
end
