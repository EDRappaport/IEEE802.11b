...
%   $Revision: 1.1.6.1 $  $Date: 2013/04/29 20:54:58 $

t =  getfmethod(this);
if(isprop(t, 'FilterStructure'))
    desString = t.FilterStructure;
    isCoupledAllpass = ...
        (strcmp(desString, 'cascadeallpass') || ...
        strcmp(desString, 'cascadewdfallpass'));
else
    isCoupledAllpass = false;
end
...
if ~returnSysObj
  ...
  % flag as an output instead of throwing an error: only true if this was
  % designed with cascadeallpass or cascadewdfallpass option
  Hs = isCoupledAllpass;
  return;
end  

if(~isCoupledAllpass)
    % Conversion from dfilt.cascade to System object only allowed when 
    % former is designed through the 'cascadeallpass' and 
    % 'cascadewdfallpass' options
    error(message('signal:dfilt:cascade:tosysobj:nogenericsupport'))
end
...
switch(desString)
    ...
    otherwise
    % No other value possible for desString at this point in the code
    assert(false)
end
