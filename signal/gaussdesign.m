...
%   seconds.
%
%   The filter is truncated to SPAN symbols and each symbol is represented
%   by SPS samples. GAUSSDESIGN designs a symmetric filter. Therefore, the
%   filter order, which is SPS*SPAN, must be even. Also, the coefficients,
%   H, are normalized so that the nominal passband gain is always equal to
%   one.
%
...
%   $Revision: 1.1.6.2.2.1 $  $Date: 2013/06/27 01:41:16 $
...
validateattributes(bt, {'double'}, ...
  {'scalar','nonempty','nonnan','real','positive'}, ...
  'gaussdesign', 'BT', 1);

validateattributes(span, {'double'}, ...
  {'scalar','nonempty','nonnan','real','positive','integer'}, ...
  'gaussdesign', 'SPAN', 2);

validateattributes(sps, {'double'}, ...
  {'scalar','nonempty','nonnan','real','positive','integer'}, ...
  'gaussdesign', 'SPS', 3);
