classdef PSKCoarseFrequencyEstimator < comm.internal.CoarseFrequencyEstimatorBase
...
properties(Access = private)
  pEstimatingFunction % Function handle to either FFT or correlation algorithm
end

properties(Access = private, Nontunable)
  pMaxLag             % For autocorrelation  
end

properties(Constant, Hidden)
  AlgorithmSet = matlab.system.StringSet({'FFT-based', 'Correlation-based'});
end


methods
  % CONSTRUCTOR
  function obj = PSKCoarseFrequencyEstimator(varargin)
  ...
  function set.ModulationOrder(obj, value)      
    validateattributes( value, { 'double' }, ...
      ...
      '', 'ModulationOrder');%#ok<EMCA>
    coder.internal.errorIf ...
      ((floor(log2(value)) ~= log2(value)), ...
      ('comm:PSKCoarseFrequencyEstimator:BadModOrder'));
    obj.ModulationOrder = value;
  end
  
  function set.MaximumOffset(obj, value)      
    validateattributes( value, { 'double' }, ...
      { 'real', 'positive', 'scalar', 'finite', 'nonnan', 'nonempty'}, ...
      '', 'MaximumOffset');%#ok<EMCA> 
    obj.MaximumOffset = value;
  end
end
...
methods(Access = protected)
  function validatePropertiesImpl(obj)
    if strcmpi(obj.Algorithm, 'FFT-based')
      validatePropertiesImpl@comm.internal.CoarseFrequencyEstimatorBase(obj);
    elseif strcmpi(obj.Algorithm, 'Correlation-based')
      coder.internal.errorIf ...
        (obj.MaximumOffset >= (obj.SampleRate/obj.ModulationOrder), ...
        ('comm:PSKCoarseFrequencyEstimator:BadMaxOffset'));
    end
  end
  
  function flag = isInactivePropertyImpl(obj, prop)
    flag = false;
    if strcmpi(prop, 'MaximumOffset') && strcmpi(obj.Algorithm, 'FFT-based')
        flag = true;
    elseif strcmpi(prop, 'FrequencyResolution') && ...
      strcmpi(obj.Algorithm, 'Correlation-based')
        flag = true;
    end
  end

  function setupImpl(obj, ~)
    setupImpl@comm.internal.CoarseFrequencyEstimatorBase(obj);
    if strcmpi(obj.Algorithm, 'FFT-based')
      obj.pEstimatingFunction = @FFTEstimateOffset;
    elseif strcmpi(obj.Algorithm, 'Correlation-based')
      obj.pEstimatingFunction = @correlationEstimateOffset;
    end

    % Find maximum offset of the tone generated by the M'th order
    % nonlinearity
    maxToneOffset = obj.MaximumOffset * obj.ModulationOrder;
      
    % Find max lag
    obj.pMaxLag = round(obj.SampleRate/maxToneOffset)-1;
  end
  
  function estFreqOffset = stepImpl(obj, x)
    estimatingFunction = obj.pEstimatingFunction;
    estFreqOffset = estimatingFunction(obj, x, obj.ModulationOrder);
  end

  function s = saveObjectImpl(obj)
    s = saveObjectImpl@comm.internal.CoarseFrequencyEstimatorBase(obj);
    if isLocked(obj)
      % Public properties handled automatically
      s.pEstimatingFunction = obj.pEstimatingFunction;
      s.pMaxLag             = obj.pMaxLag;
    end
  end
  ...
  function loadObjectImpl(obj, s, wasLocked)
    if wasLocked
      % Public properties handled automatically
      obj.pEstimatingFunction = s.pEstimatingFunction;
      obj.pMaxLag             = s.pMaxLag;
    end
    % Call the base class method
    loadObjectImpl@comm.internal.CoarseFrequencyEstimatorBase(obj, s, wasLocked);
  end
...
function estFreqOffset = correlationEstimateOffset(obj, x, ~)
  % Raise the input signal to the M'th power
  raisedSignal = x .^ obj.ModulationOrder;

  % Perform auto-correlation and find the frequency offset
  R = xcorr(raisedSignal, obj.pMaxLag, 'unbiased');
  estFreqOffset = obj.SampleRate * angle(sum(R(obj.pMaxLag+2:end))) / ...
      ((obj.pMaxLag+1) * obj.ModulationOrder*pi);
end
