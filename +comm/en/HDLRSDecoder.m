classdef HDLRSDecoder< handle
%HDLRSDecoder Decode data using a Reed-Solomon decoder
%   H = comm.HDLRSDecoder returns a block decoder System object, H,
%   that performs Reed-Solomon (RS) decoding.
%
%   H = comm.HDLRSDecoder(Name,Value) creates an RS decoder object,
%   H, with the specified property Name set to the specified
%   Value. You can specify additional name-value pair arguments in
%   any order as (Name1,Value1,...,NameN,ValueN).
%
%   H = comm.HDLRSDecoder(N,K,Name,Value) creates an RS decoder
%   object, H, with the CodewordLength property set to N, the
%   MessageLength property set to K, and other specified property
%   Names set to the specified Values.
%
%   Step method syntax:
%
%   [Y,startOut,endOut,validOut,errOout] = step(H,X,startIn,EndIn,validIn) 
%   decodes the input data, X, and returns the encoded data, Y. 
%
%   HDLRSDecoder methods:
%
...
%   NumErrorsOutputPort       - Enable number of corrected errors output
%
%   % Example:
%   %   RS-encoded and decode a DVD-II standard packet of random data
%
...
%     [decOut(ii), decStartOut(ii), decEndOut(ii), decValidOut(ii), decErrOut(ii)] = step(hHDLDec, encOut(ii), startOut(ii), endOut(ii), validOut(ii));
%   end
%   % Check the result:
%   assert(all(dataIn(1:188) == decOut(decValidOut)'))
%
%   See also comm.HDLRSEncoder, comm.RSDecoder.

  
%   Copyright 2012-2013 The MathWorks, Inc.

    methods
        function out=HDLRSDecoder
            %HDLRSDecoder Decode data using a Reed-Solomon decoder
            %   H = comm.HDLRSDecoder returns a block decoder System object, H,
            %   that performs Reed-Solomon (RS) decoding.
            %
        ...
        function GFInverse(in) %#ok<MANU>
        end

        function GFMultiply(in) %#ok<MANU>
        end

        function chien(in) %#ok<MANU>
        end
    ...
    properties
        ...
        %NumErrorsOutputPort Enable number of errors output
        %   When you set this property to true, the step method outputs number of
        %   corrected errors. The number of corrected errors is not valid when errOut
        %   is asserted, since there were more errors than could be corrected.  The
        %   default is false.
        NumErrorsOutputPort;
