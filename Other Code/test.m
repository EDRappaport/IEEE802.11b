...
DBPSK_Mod = comm.DBPSKModulator;    
DBPSK_Demod = comm.DBPSKDemodulator;  
...
for i=1:length(SNR)
    ...
    for packet=1:NumPackets
        ...
        reset(TxFilter);reset(RxFilter);reset(DBPSK_Mod);reset(DBPSK_Demod);
        ...
        %Demodulate Data
        RxBits = step(DBPSK_Demod,RxSymbols);
        
        %%%%%%%%%%%%BER Calculation%%%%%%%%%%%%%%%%%%%         
        TotalBits = TotalBits+length(RxBits)-TotalDelayInBits;
        ErrorBits = ErrorBits+sum(TxBits(1:end-TotalDelayInBits)~=...
...
title('BER for IEEE 802.11b 1Mbps through an AWGN Channel');
