...
%DigitalUpConverter Digitally up convert input signal
...
%
%   The DigitalUpConverter object supports fixed-point operations. For
...
properties (Dependent, Nontunable)
  ...
  %   stages. The scalar value you specify cannot be a prime number.
  %
...
methods
  ...
  function outputStruct = getFilters(obj,varargin)
    ...
    if isLocked(obj)
      ...
      outputStruct.FirstFilterStage = c{1};
      outputStruct.SecondFilterStage = c{2};
      outputStruct.CICInterpolator = c{3};
      outputStruct.CICNormalizationFactor = c{4};
    end
  end
  %------------------------------------------------------------------------
  function outputStruct = getFilterOrders(obj,varargin)
    ...
    if ~isempty(c{1})
      outputStruct.FirstFilterOrder = length(c{1}.Numerator)-1;
    else
    ...
    if strcmpi(obj.pFilterDesigner.Arithmetic, 'fixed');
      outputStruct.NumCICSections = c{3}.NumSections;
    else
      outputStruct.NumCICSections = ...
        (length(c{3}.Numerator)-1)/(interpFactors(end)-1);
    end
  end  
  %------------------------------------------------------------------------
  ...
  function set.InterpolationFactor(obj,val)
    ...
    if (length(val) ~= 1) && (length(val) ~= 2) && (length(val) ~= 3)
      error(message('dsp:system:DigitalUpConverter_invalidInterpFactor'))
    end
    
    if (length(val) == 1)
      if val < 2
        error(message('dsp:system:DigitalUpConverter_invalidInterpFactorScalar'))
      end
      if isprime(val)
        error(message('dsp:system:DigitalUpConverter_invalidInterpFactorScalarPrime'))        
      end
    end
...
%---------------------------------------------  
  ...
  function set.FirstFilterOrder(obj,val)
    ...
    if mod(val,2) ~= 0
      error(message('dsp:system:DigitalUpConverter_oddFirstFilterOrder'))
    end
...
methods (Hidden)
  function hdlStruct = getFixedPointParameters(obj)
    ...
    if ~isLocked(obj)
      error(message('dsp:system:DigitalUpConverter_invalidObjectState'));
    end
...
methods (Access = protected)
  ...
  function validateInputsImpl(obj, x)
    ...
    if ~iscolumn(x) || isempty(x)
      matlab.system.internal.error('MATLAB:system:inputMustBeColVector', ...
        'X');
    end
  ...
  function setupImpl(obj,~,varargin)
    ...
    % factor] output.
    if obj.pIsInputFixedPoint
      obj.pInitCICNormOutput = fi(zeros(sz),obj.pFiltersOutputNumericType);
    else
    ...
    if obj.pIsInputFixedPoint
      ...
      obj.pInitOscillatorOutput = ...
        fi(zeros(sz), obj.pOscillatorOutputNumericType);      
    else
      obj.pInitMixerOutput = zeros(sz, obj.pInputDataType);      
      obj.pInitOscillatorOutput = zeros(sz, obj.pInputDataType);      
    end
  ...
  function checkFactorLength(obj)
    ...
    if length(obj.InterpolationFactor)~= 3 && length(obj.InterpolationFactor)~= 2
      error(message('dsp:system:DigitalUpConverter_interpFactorNotVector'));
    end
  end  
  %------------------------------------------------------------------------  
  function checkFactorValues(obj)
    ...
    if strcmp(obj.FilterSpecification,'Design parameters')
      if length(obj.InterpolationFactor) == 3
        if (obj.InterpolationFactor(1) > 2)
          error(message('dsp:system:DigitalUpConverter_invalidInterpFactorFirstElement'));
        end
      end
      
      if length(obj.InterpolationFactor) > 1
        if any(obj.InterpolationFactor(2:end) < 2)
          error(message('dsp:system:DigitalUpConverter_invalidInterpFactorSecondThirdElements'));
        end
      end
    else
      if length(obj.InterpolationFactor) > 1
        if any(obj.InterpolationFactor(end) < 2)
          error(message('dsp:system:DigitalUpConverter_invalidInterpFactorThirdElement'));            
        end
      end
    end    
  end
  ...
  function checkFsFc(obj)
    ...
    if obj.SampleRate*prod(obj.InterpolationFactor) < 2*obj.CenterFrequency
      error(message('dsp:system:DigitalUpConverter_InvalidFs'));
    end
  end   
  %------------------------------------------------------------------------  
...
methods (Access = private)  
  ...
  function designInterpolatorCascade(obj)
    ...
    if strcmpi(obj.pArithmetic,'fixed')
      inputFixedPointArguments = ...
        {obj.pInputNumericType,obj.pFiltersOutputNumericType};      
    end    
    if obj.pFilterDesigner.FilterDesignIsAvailable && ...
    ...
    obj.pStage1 = filterCell{1};
    obj.pCICComp = filterCell{2};
    obj.pCIC = filterCell{3};       
    ...
    if obj.pIsInputFixedPoint      
      ...
      % Output data types and filter output data types
      if  ~isFirstStageBypassed(obj.pFilterDesigner)
        % Set first stage output data type to pFiltersOutputNumericType
        obj.pStage1.OutputDataType = 'Custom';
        obj.pStage1.CustomOutputDataType = obj.pFiltersOutputNumericType;        
      end
            
      % Set CIC compensator output data type to pFiltersOutputNumericType
      obj.pCICComp.OutputDataType = 'Custom';
      obj.pCICComp.CustomOutputDataType = obj.pFiltersOutputNumericType;
      ...
      obj.pRemNormFactor = fi(...
       ...
       % WL at CIC output
      numCICOutputBits = obj.pFiltersOutputNumericType.WordLength + ...
      ...
      obj.pReinterpretCastNumericType = numerictype(true,numCICOutputBits,...
        obj.pFiltersOutputNumericType.FractionLength+numCICOutputShifts);
